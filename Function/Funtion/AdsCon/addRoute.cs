...
public sealed class BeckhoffRouteConnector
{
    ...
    public static bool CheckRoute(BeckhoffModel request)
    {

        var resultVar = new BeckhoffRouteConnectRequest() 
        {
        ...
        PowerShellResult testResult = ExecuteTestRoute(resultVar);
        if (testResult.Output.Contains("Ok"))
            return true;
        return false;
...
";
    ...
    private static PowerShellResult ExecuteTestRoute(BeckhoffRouteConnectRequest request)
    {
        string script = $@"
$ErrorActionPreference = 'Stop'
$ProgressPreference = 'SilentlyContinue'
