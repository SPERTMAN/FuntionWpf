...
public record struct HotkeyModel
{
    ...
    public HotkeyModel(string hotkeyString) => Parse(hotkeyString);
    ...
    private void Parse(string hotkeyString)
    {
        ...
        if (keys.Count == 1)
        {
            ...
            CharKey = specialSymbolPair.Value.Value != null
                ? specialSymbolPair.Value.Key
                : Enum.TryParse(charKey, out Key keyValue) ? keyValue : Key.None;
        }
    }

    public override string ToString() => string.Join(" + ", EnumerateDisplayKeys());
    ...
    public bool Validate(bool validateKeyGestrue = false)
    {
        switch (CharKey)
        {
            ...
            default:
                ...
                return ModifierKeys != ModifierKeys.None || !IsPrintableCharacter(CharKey);
        }
    }
    ...
    public override int GetHashCode() => HashCode.Combine(ModifierKeys, CharKey);
}